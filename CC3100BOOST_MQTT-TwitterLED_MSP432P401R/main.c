 ...
 */
#define SSID_NAME       "secucrypto"       /* Access point name to connect to. */
#define SEC_TYPE        SL_SEC_TYPE_OPEN     /* Security type of the Access piont */
#define PASSKEY         "000"   /* Password in case of secure AP */
#define PASSKEY_LEN     pal_Strlen(PASSKEY)      /* Password length in case of secure AP */

#define UDP_PORT_NUM        (8081)
#define UDP_BUF_SIZE        (256)
#define UDP_PACKET_COUNT    (1000)
#define UDP_SERVER_IP       SL_IPV4_VAL(192,168,1,115)
...
/* Configuration of the crypto 				*/
#define CRPTO_KEY			"AZERTYAZERTYAZE"
#define CRPTO_KEYLEN		AES256_KEYLENGTH_128BIT
#define CRPTO_IV			"000000000000000"
#define CRPTO_MSGLEN		(16)
...
void initBoardAndWifiConnect(void)
{
    _i32 retVal = -1;
...
void udp_socket_create( void )
{
    // Create the UDP socket
    socket_id = socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP );
    if ( socket_id == -1 )
    {
        CLI_Write( "ERROR socket creation failed" );
    }
    // zero out the structure
    memset( ( char * ) &si_me, 0, sizeof( si_me ) );
    si_me.sin_family = AF_INET;
    si_me.sin_port = htons( UDP_PORT_NUM );
    si_me.sin_addr.s_addr = sl_Htonl(UDP_SERVER_IP);

    //bind socket to port
    if( bind( socket_id , ( struct sockaddr * )&si_me, sizeof( si_me ) ) == -1 )
    {
        CLI_Write( "ERROR bind failed" );
    }
}

void udp_socket_send( char * data, int length )
{
    if ( sendto(socket_id, data, length, 0, &si_me, sizeof( si_me )) == -1 )
        CLI_Write("ERROR sendto()");
}
...
{
    // Init
    initBoardAndWifiConnect();
    udp_socket_create();

    // Message to be sent
    unsigned char message[UDP_BUF_SIZE] = "#S8";
    unsigned char encryptedMessage[CRPTO_MSGLEN];
    uint8_t i=0;

    for(i=0;i<16;i++)
    {
    	message[i] ^= CRPTO_IV[i];
    }

    AES256_reset(AES256_MODULE);

    AES256_setCipherKey(AES256_MODULE, CRPTO_KEY, CRPTO_KEYLEN);

    AES256_encryptData(AES256_MODULE, message,
    		encryptedMessage);

    // Main loop
    while(1){
        CLI_Write("Sending packet\n");
        udp_socket_send( (char*) encryptedMessage, 16 );
        while(S1buttonDebounce == 0);
    }
}
...
{
    CLI_Write("\n\r\n\r");
    CLI_Write(" Send a message on UDP port 8081");
    CLI_Write("\n\r*******************************************************************************\n\r");
}
